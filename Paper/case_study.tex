...
The agent has the choice of moving in 4 directions - North, South, East, and West or staying still. The motion is stochastic - i.e, it has a probability of slip. For example, if it chooses to move north, it has a probability to 'slip' and move to a state north east or north west. 
...
\subsection{Mars rover case study}\label{sec:marsexp}
Now, we present the example of the Mars rover discussed in section \ref{sec:casestudy}. Figure \ref{fig:mars} shows a simple representation of the environment. We start with a rough knowledge of the environment. We know the red region is impassable terrain. We also know that there is a region with a high distribution of obstacles and one with a low distribution of obstacles. All other regions are assumed to be obstacle free.

\begin{figure}
\subfloat[Representation of a martian environment for case study.\label{fig:mars}]{
	\includegraphics[scale=0.08]{casestudygrid.png}
}
\end{figure}

The LTL specification is again $\lnot$'crash' $\LTLuntil$ 'goal' where the goal is the green area. This time the state in the MDP is given by $(x,y,(o_i,...o_n))$ where $(o_1,..,o_n) \in \mathbb{B}^n$ are boolean variables representing where $o_i$ evaluates to True if an obstacle is present at state $i$. We assume obstacles can only be present in the two regions shown in figure \ref{fig:mars}. 
